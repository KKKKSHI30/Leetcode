...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    def __setitem__(self, k, v):
        ...
        if self.is_empty():
            ...
            self._rebalanced_insert(leaf)   # hook for balanced tree subclasses
    ...
    def _rebalance(self, p):
        while p is not None:
            ...
            if not self._isbalance(p):    # imbalance detected!
                ...
                self._recompute_hieght(self.left(p))
                self._recompute_hieght(self.right(p))
            self._recompute_hieght(p)    # adjust for recent changes
            if p._node._height == old_height:   # has height chnaged?
        ...
        if self.is_root(p):
            ...
            if self._is_red(parent):   # double red problem
                ...
                if not self._is_red(uncle):    # Case 1: misshapen 4-node
                    ...
                    self._set_black(self.tight(grand))   # its children become black
